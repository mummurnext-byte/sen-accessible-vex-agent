...
class SilentSparksDuckHost(Scene):
    ...
    def duck(self, pose: str, height: float) -> ImageMobject:
        asset = Path(__file__).resolve().parent.parent / "assets" / "duck_poses" / f"{pose}.png"
        return ImageMobject(str(asset)).set_height(height)
    ...
    def silent_hook(self) -> VGroup:
        teacher = self.duck("pointing", 3.25).move_to([-3.25, 0.45, 0])
    ...
    def input_scene(self, previous: VGroup) -> VGroup:
        ...
        teacher = self.duck("walking", 3.35).move_to([-3.0, 0.2, 0])
    ...
    def brain_scene(self, previous: VGroup) -> VGroup:
        ...
        teacher = self.duck("magnifying", 2.65).move_to([-3.35, 0.45, 0])
        ...
        group = Group(title, teacher, shadow, blocks, lens, handle, scanner, caption)
    ...
    def simulation_scene(self, previous: VGroup) -> VGroup:
        ...
        finger = self.duck("pressing", 2.25).move_to([-3.05, 1.2, 0])
        ...
        target_label = Text("300 mm", font="Arial", font_size=23, weight=BOLD, color=BLACK).move_to([3.42, -0.95, 0])
        robot = self.robot().scale(0.82).move_to([1.25, 0.65, 0])
        observer = self.duck("thinking", 1.45).move_to([3.62, 3.25, 0])
        caption = self.caption(
        ...
        group = Group(title, divider, button, check, finger, wait_block, true_text, lane, target, target_label, robot, observer, caption)
        self.play(FadeIn(title), Create(divider), FadeIn(caption), run_time=0.8)
        self.play(FadeIn(button, scale=0.55), FadeIn(check), FadeIn(finger, shift=RIGHT * 0.3), FadeIn(wait_block), Create(lane), Create(target), FadeIn(target_label), FadeIn(observer), run_time=1.7)
    ...
    def prediction_scene(self, previous: VGroup) -> VGroup:
        ...
        teacher = self.duck("thinking", 3.05).move_to([-3.0, 1.2, 0])
        ...
        group = Group(title, teacher, shadow, question, old, lane, marker_300, marker_500, robot, option_a, option_b, no, yes, caption)
